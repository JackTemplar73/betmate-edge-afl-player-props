...
| 93.5 | A_BET | Archie Roberts | Disposals | Over | 33.667 | 29.5 | 1.83 | PointsBet (AU) | 37.4% | 1.00u |
...
- What it means in plain English: the model still likes this side after smoothing recent form and pulling the player back toward his longer-run baseline, so this is not just a one-hot-game bet.
- Why it made the card: Model data projects 22.304 disposals against a 25.5 line, creating a +3.2 stat gap for the under. The Markov state path is Strong -> Dominant -> Mispriced -> Elite, meaning the row moves from projection support into a positive probability state, then through the price/EV state, and finishes with 95.0 QI. Model probability is 71.4% versus market 48.8%, producing 36.4% EV.
...
### Archie Roberts Disposals Over 29.5 @ 1.83 (PointsBet (AU))
- QI: 93.5
- Markov path: -
- Average bettor read: the adjusted model line is `33.667` against a market line of `29.5`. The raw season line was `32.455`, and the Kalman/reversion step moved it by `1.212`. That leaves the model showing `75.1%` win probability and `37.4%` EV at `1.83`.
- What it means in plain English: the model still likes this side after smoothing recent form and pulling the player back toward his longer-run baseline, so this is not just a one-hot-game bet.
...
- What it means in plain English: the model still likes this side after smoothing recent form and pulling the player back toward his longer-run baseline, so this is not just a one-hot-game bet.
- Why it made the card: Model data projects 4.606 tackles against a 3.5 line, creating a +1.1 stat gap for the over. The Markov state path is Strong -> Dominant -> Mispriced -> Elite, meaning the row moves from projection support into a positive probability state, then through the price/EV state, and finishes with 91.5 QI. Model probability is 63.1% versus market 34.5%, producing 83.1% EV.
