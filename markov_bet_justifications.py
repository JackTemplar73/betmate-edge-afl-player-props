#!/usr/bin/env python3
"""Create Markov-style row-by-row justifications for Odds API + model-data bets."""
...
def commentary(row: dict[str, str]) -> list[str]:
    ...
    if market == "Goals":
        ...
        distribution_note = (
            "Tackles are modelled as a pressure/contact state. The bet improves when the Model data tackle rate is "
    ...
    if p_state == "Strong":
        projection_text = (
            f"The chain opens in a strong projection state: Model data has {row['player']} at {projection:.2f} "
            f"against a {line:.1f} line, so the {side} starts with a {gap:+.2f} {stat_unit} cushion."
        )
    elif p_state == "Lean":
        projection_text = (
            f"The chain opens in a lean projection state: Model data is on the right side, but the cushion is "
            f"only {gap:+.2f}. This is why the row needs the market price to do more of the work."
        )
    else:
        projection_text = (
            f"The projection state is not dominant. Model data is close to the line, so this row depends more "
...
def write_outputs(rows: list[dict[str, str]]) -> None:
    ...
    if not out_rows:
        lines.append("No A_BET or B_BET rows were available on this refresh.")
        lines.append("")
    ...
    verbose = [
        ...
        "",
        "Scope: every A_BET and B_BET from the live Odds API + Model data sheet.",
        "",
    ...
    if not out_rows:
        verbose.append("No A_BET or B_BET rows were available on this refresh.")
        verbose.append("")
    for idx, row in enumerate(out_rows, start=1):
        verbose.extend(
            [
