...
def build_pdf(rows: list[dict[str, str]]) -> None:
    ...
    story.append(
        para(
            f"Generated {datetime.now().strftime('%Y-%m-%d %H:%M')} from WheeloRatings + Odds API. "
            "Scope: Western Bulldogs v Melbourne only; GWS v Brisbane returned no live player prop bookmakers. "
    ...
    story.append(
        para(
            "A_BET means strong Wheelo support plus a clear model edge and high QI. B_BET means positive EV with one softer state, usually a smaller projection cushion or thinner price. "
    ...
    story.append(
        para(
            "Sources: WheeloRatings current player pack and team context, The Odds API AU bookmaker markets. This is a model output, not a guarantee; prices can move and late team/role news can invalidate an edge.",
            body,
        )
    )
