...
def tracking_tab_table(rows: list[dict[str, str]]) -> str:
    ...
    out = [
        "<table><thead><tr>"
        "<th>QI</th><th>Team</th><th>Player</th><th>Market</th><th>Side</th><th>Model Line</th>"
        "<th>Market Line</th><th>Model Prob</th><th>Price</th><th>Bookie</th><th>Prob Edge</th><th>Stake</th><th>Result</th>"
        "</tr></thead><tbody>"
    ]
    for row in rows:
        edge = (to_float(row.get("model_probability")) or 0) - (to_float(row.get("market_probability")) or 0)
...
def main() -> None:
    ...
    if args.game:
        portfolio = [
            row for row in scored_rows
            if row.get("signal") in {"A_BET", "B_BET"}
    ...
    if not args.game:
        suppressed_goal = [
            row for row in scored_rows
            if row.get("signal") in {"A_BET", "B_BET"} and row.get("portfolio_selection") == "SUPPRESSED_GOAL_CAP"
        ]
        suppressed_goal.sort(key=lambda row: (float(row.get("selection_rank") or 9999), -(to_float(row.get("live_qi")) or 0)))
        suppressed_portfolio = [
            row for row in scored_rows
            if row.get("signal") in {"A_BET", "B_BET"} and row.get("portfolio_selection") == "SUPPRESSED_PORTFOLIO_CAP"
        ]
